defmodule SmartmeterWeb.PageController do
 ...
 # import Logger
  ...
  defp assign_from_cache(conn, key, prefix \\"", postfix \\ "") when is_atom(key) do
  	value = case ConCache.get(:cache, key) do
  ...
  def mbus_device_measurements do
    ...
    |> Enum.map(fn channel -> {channel,
                         {ConCache.get(:cache, String.to_atom("mbus_device_measurement_#{channel}")),
                          ConCache.get(:cache, String.to_atom("mbus_device_measurement_timestamp_#{channel}"))}} end)
    |> Map.new
  end

  defp get_from_channel(_channel) do
    [] #TODO
  end
  ...
  def index(conn, _params) do
    information = get_from_channel(0)
    conn
      |> assign(:information, information)
      |> assign(:mbus_devices, get_channels())
